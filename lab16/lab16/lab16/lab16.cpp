    ...
    do {
        ...
        case 1: {
            ...
            for (int i = 0; i < N; ++i) {
                for (int j = 0; j < M; ++j) {
                    cout << "matrix[" << i << "][" << j << "]: ";
                    cin >> matrix[i][j]; // Ввод элементов массива
                }
            }
            ...
            deleteDynamicArray(matrix, N); // Освобождение памяти
            break;
        }
        case 2: {
            int numStrings;
            ...
            cin >> numStrings;
            cin.ignore();
            string* strings = new string[numStrings]; // Создание массива строк

            cout << "Введите строки текста:" << endl;
            for (int i = 0; i < numStrings; ++i) {
                cout << "Строка " << i + 1 << ": ";
                getline(cin, strings[i]); // Ввод строк текста
            }

            printStringsStartingWithB(strings, numStrings); // Вывод строк, начинающихся с 'b'
